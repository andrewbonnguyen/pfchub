...
const ToastContainer = ({ toasts }) => (
  <div style={{ position: "fixed", bottom: 20, right: 20, zIndex: 9999, display: "flex", flexDirection: "column", gap: 8, pointerEvents: "none" }}>
    {toasts.map(t => (
      <div key={t.id} style={{
        padding: "10px 16px", borderRadius: 6, maxWidth: 380, pointerEvents: "all",
        fontFamily: "var(--font-mono)", fontSize: 12, fontWeight: 500,
        animation: "toastSlide .2s ease", boxShadow: "0 8px 24px rgba(0,0,0,.6)",
        background: t.type === "error" ? "#190810" : t.type === "success" ? "#071510" : "#090e1c",
        borderLeft: `3px solid ${t.type === "error" ? "var(--red)" : t.type === "success" ? "var(--green)" : "var(--accent)"}`,
        color: t.type === "error" ? "#f08090" : t.type === "success" ? "#40e8a0" : "var(--text-2)",
      }}>{t.msg}</div>
    ))}
  </div>
);
...
const Badge = ({ label, value, color = "var(--accent)" }) => (
  <div style={{ display: "inline-flex", gap: 5, alignItems: "center", background: `${color}14`, border: `1px solid ${color}30`, borderRadius: 5, padding: "3px 9px" }}>
...
const Btn = ({ children, onClick, disabled = false, variant = "ghost", color, size = "md", full = false }) => {
  const c = color || (variant === "danger" ? "var(--red)" : variant === "solid" ? "var(--accent)" : "var(--text-3)");
  return (
    <button onClick={onClick} disabled={disabled} style={{
      display: "inline-flex", alignItems: "center", justifyContent: "center",
      gap: size === "sm" ? 5 : 7, padding: size === "sm" ? "5px 11px" : "8px 15px",
      fontSize: size === "sm" ? 12 : 13, fontFamily: "var(--font-ui)", fontWeight: 600,
      ...
      color: variant === "solid" ? (color === "var(--accent)" ? "#000" : "#fff") : c,
    }}>{children}</button>
  );
};
...
const inputBase = {
  ...
  borderRadius: 6, color: "var(--text-1)", fontFamily: "var(--font-ui)", fontSize: 13,
...
const SelectInput = ({ value, onChange, children }) => (
  <select value={value} onChange={onChange} style={{
    ...inputBase, padding: "8px 32px 8px 11px", cursor: "pointer", appearance: "none",
...
const TextArea = ({ value, onChange, placeholder, rows = 3, mono = false }) => (
  <textarea value={value} onChange={onChange} placeholder={placeholder} rows={rows}
    style={{ ...inputBase, padding: "8px 11px", resize: "vertical", fontFamily: mono ? "var(--font-mono)" : "var(--font-ui)", fontSize: 12 }}
    onFocus={e => (e.target.style.borderColor = "var(--accent)")}
    onBlur={e => (e.target.style.borderColor = "var(--border-1)")} />
);
  ...
  <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
    {tabs.map(([id, label]) => (
      <button key={id} onClick={() => onChange(id)} style={{
        ...
        fontFamily: "var(--font-ui)", fontWeight: active === id ? 600 : 400, transition: "all .15s",
      }}>{label}</button>
    ))}
  </div>
);
...
const Card = ({ children, style = {} }) => (
  <div style={{ background: "var(--bg-2)", border: "1px solid var(--border-1)", borderRadius: 10, padding: 18, ...style }}>
    {children}
  </div>
);

const CardTitle = ({ children }) => (
  <div style={{ fontSize: 10, fontFamily: "var(--font-mono)", color: "var(--text-3)", textTransform: "uppercase", letterSpacing: 1, marginBottom: 12 }}>
    {children}
  </div>
);
...
const ProgressBar = ({ pct }) => (
  <div style={{ background: "rgba(255,255,255,.06)", borderRadius: 999, height: 5, overflow: "hidden" }}>
    <div style={{ width: `${Math.min(100, pct || 0)}%`, height: "100%", background: "var(--accent)", borderRadius: 999, transition: "width .5s ease", boxShadow: "0 0 8px var(--accent)80" }} />
  </div>
);
...
const actionBtnStyle = (color) => ({
  background: `${color}14`, border: `1px solid ${color}28`, borderRadius: 5,
  padding: "4px 7px", cursor: "pointer", color, display: "flex", alignItems: "center",
  transition: "background .13s",
});

const SRC_COLOR = { manual: "var(--purple)", scanned: "var(--yellow)", imported: "var(--accent)" };
...
const TranslatePage = ({ toast }) => {
  ...
  useEffect(() => {
    ...
    es.onmessage = ({ data }) => {
      try {
        ...
        if (d.type === "done") {
          setDone(d); setProgress({ ...d, percent: 100, phase: "done" }); es.close();
          toast(`✅ Xong! ${d.translated} cells · ${d.geminiCalls || 0} Gemini calls`, "success");
          api("/translate/cache-stats").then(setCacheStats).catch(() => {});
        }
        if (d.type === "error") {
          toast("❌ " + d.error, "error"); es.close(); setJobId(null); setProgress(null);
        }
      } catch {}
    };
    return () => es.close();
  ...
  const startTranslation = async () => {
    ...
    setProgress({ phase: "init", percent: 0, translated: 0, total: 0, geminiCalls: 0 }); setDone(null);
    const fd = new FormData(); fd.append("file", file); fd.append("targetLang", lang);
    try {
      const { jobId: id } = await api("/translate/start", { method: "POST", body: fd });
      setJobId(id); toast("🚀 Bắt đầu dịch...", "info");
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 290px", gap: 18, alignItems: "start" }}>
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <div style={{ display: "flex", gap: 12, alignItems: "flex-end" }}>
            ...
            <Btn variant="solid" color="var(--accent)" onClick={startTranslation} disabled={!file || busy}>
              {busy ? <><span className="spin-icon">⟳</span> Đang dịch…</> : <><Icon name="zap" /> Bắt Đầu Dịch</>}
            </Btn>
          </div>
            ...
            <Card>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
                <span style={{ fontSize: 13, color: "var(--text-2)" }}>
                  {progress.phase === "done" ? "✅ Hoàn thành" : progress.phase === "reading" ? "📖 Đọc file…" : progress.phase === "init" ? "⏳ Chuẩn bị…" : "⚙️ Đang dịch…"}
                </span>
                <span style={{ fontFamily: "var(--font-mono)", color: "var(--accent)", fontWeight: 700, fontSize: 14 }}>{progress.percent || 0}%</span>
              </div>
          ...
          {/* Download */}
          {done?.outputFile && (
            <a href={`${API}/translate/download/${done.outputFile}`} download className="download-banner">
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          <Card>
            ...
            <TextArea value={testInput} onChange={e => setTestInput(e.target.value)} placeholder="Nhập EN / ZH để dịch thử..." mono />
            <div style={{ marginTop: 8 }}>
              <Btn full onClick={runTest} disabled={testBusy || !testInput.trim()}>
                {testBusy ? "Đang dịch…" : "▶  Dịch thử"}
              </Btn>
            </div>
            {testResult && (
              <div style={{ marginTop: 10, padding: "10px 12px", background: "rgba(0,0,0,.3)", borderRadius: 6, borderLeft: "2px solid var(--accent)" }}>
                <div style={{ color: "var(--accent)", fontSize: 12, fontWeight: 600, lineHeight: 1.6, wordBreak: "break-word" }}>{testResult.translated}</div>
                <div style={{ fontSize: 10, color: "var(--text-3)", fontFamily: "var(--font-mono)", marginTop: 6 }}>
                  {testResult.fromGlossary ? `📚 Glossary (${testResult.matchType})` : testResult.fromCache ? "⚡ Cache" : "🤖 Gemini API"}
                </div>
              </div>
            )}
          </Card>
          ...
          <div style={{ background: "rgba(255,200,40,.04)", border: "1px solid rgba(255,200,40,.14)", borderRadius: 8, padding: 14 }}>
            <div style={{ fontSize: 11, fontFamily: "var(--font-mono)", color: "var(--yellow)", marginBottom: 5 }}>💡 Tối ưu quota</div>
            <div style={{ fontSize: 12, color: "var(--text-3)", lineHeight: 1.65 }}>
              Import file đã dịch mẫu vào <strong style={{ color: "var(--text-2)" }}>Từ Điển</strong> để tăng Glossary hits, giảm Gemini API calls.
            </div>
          </div>
        </div>
      </div>
...
const GlossaryPage = ({ toast }) => {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      setEntries(g.entries); setPages(g.pages || 1); setTotal(g.total); setStats(s);
    } catch (e) { toast("Lỗi tải từ điển: " + e.message, "error"); }
    setLoading(false);
  }, [page, search]);
  ...
  const handleAddEntry = async () => {
    if (!newRow.en && !newRow.zh) return toast("Nhập ít nhất EN hoặc ZH", "error");
    try {
      await api("/glossary/entry", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      setNewRow({ en: "", zh: "", vi: "" }); setAddOpen(false); loadData();
    } catch (e) { toast("Lỗi thêm: " + e.message, "error"); }
  };

  const handleDelete = async (id) => {
    if (!confirm("Xóa từ này?")) return;
    try {
      await api(`/glossary/entry/${id}`, { method: "DELETE" });
      toast("Đã xóa", "success"); loadData();
  ...
  const handleScan = async () => {
    ...
    } catch (e) { toast("Lỗi quét: " + e.message, "error"); }
    setScanBusy(false);
  };
  ...
  const handleImport = async () => {
    ...
    fd.append("sourceCol", impCols.src);
    fd.append("translatedCol", impCols.tgt);
    fd.append("lang", "vi");
    try {
    ...
    } catch (e) { toast("Lỗi import: " + e.message, "error"); }
    setImpBusy(false);
  };
  ...
  const exportCSV = async () => {
    try {
      ...
      a.click(); toast("✅ Đã xuất CSV", "success");
    } catch (e) { toast("Lỗi xuất: " + e.message, "error"); }
  };
  ...
  const CellInput = ({ value, onChange, highlight }) => (
    <input value={value} onChange={onChange} style={{
      ...
      color: "var(--text-1)", fontFamily: "var(--font-mono)", fontSize: 12, outline: "none",
    }} />
  );
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
      ...
      {/* Add form */}
      {addOpen && (
        <div style={{ background: "rgba(140,80,255,.06)", border: "1px solid rgba(140,80,255,.2)", borderRadius: 8, padding: 16 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10 }}>
            {[["English", "en"], ["中文", "zh"], ["Tiếng Việt", "vi"]].map(([label, key]) => (
              <div key={key}>
                ...
                <TextInput value={newRow[key]} onChange={e => setNewRow(p => ({ ...p, [key]: e.target.value }))} />
              </div>
            ))}
          </div>
        ...
        <>
          <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
            <div style={{ flex: 1, position: "relative", display: "flex", alignItems: "center" }}>
              <span style={{ position: "absolute", left: 11, color: "var(--text-3)", display: "flex" }}><Icon name="search" size={14} /></span>
              <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Tìm EN · ZH · VI…"
                style={{ ...inputBase, padding: "8px 32px 8px 34px" }}
                onFocus={e => (e.target.style.borderColor = "var(--accent)")}
                onBlur={e => (e.target.style.borderColor = "var(--border-1)")} />
              {search && (
            ...
            <span style={{ fontSize: 11, color: "var(--text-3)", fontFamily: "var(--font-mono)", whiteSpace: "nowrap" }}>{total} từ</span>
          </div>
              ...
              <thead>
                <tr style={{ background: "rgba(0,0,0,.3)" }}>
                  {["English", "中文", "Tiếng Việt", "Nguồn", ""].map((h, i) => (
                    <th key={i} style={{ padding: "9px 13px", textAlign: "left", fontSize: 10, fontFamily: "var(--font-mono)", color: "var(--text-3)", textTransform: "uppercase", letterSpacing: .7, fontWeight: 500, borderBottom: "1px solid var(--border-1)", whiteSpace: "nowrap" }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                    ...
                    : entries.map(e => (
                      <tr key={e.id} style={{ borderBottom: "1px solid rgba(25,30,48,.7)" }}
                        ...
                        {editRow?.id === e.id ? (
                          <>
                            {["en", "zh", "vi"].map(k => (
                              <td key={k} style={{ padding: "6px 10px" }}>
                                <CellInput value={editRow[k] || ""} onChange={ev => setEditRow(r => ({ ...r, [k]: ev.target.value }))} highlight={k === "vi"} />
                              </td>
                            ))}
                          ...
                          <>
                            <td style={{ padding: "9px 13px", color: "var(--text-1)", fontSize: 13 }}>{e.en || <span style={{ color: "var(--border-2)" }}>—</span>}</td>
                            <td style={{ padding: "9px 13px", color: "var(--yellow)", fontSize: 13 }}>{e.zh || <span style={{ color: "var(--border-2)" }}>—</span>}</td>
                            ...
                            <td style={{ padding: "9px 13px" }}>
                              <span style={{ fontSize: 10, fontFamily: "var(--font-mono)", background: `${SRC_COLOR[e.source] || "var(--text-3)"}18`, border: `1px solid ${SRC_COLOR[e.source] || "var(--text-3)"}30`, color: SRC_COLOR[e.source] || "var(--text-3)", borderRadius: 4, padding: "2px 8px" }}>{e.source}</span>
                            </td>
                            <td style={{ padding: "9px 13px" }}>
                              <div style={{ display: "flex", gap: 4 }}>
                                <button onClick={() => setEditRow({ id: e.id, en: e.en || "", zh: e.zh || "", vi: e.vi || "" })} style={actionBtnStyle("var(--purple)")}><Icon name="edit" size={13} /></button>
          ...
          {pages > 1 && (
            <div style={{ display: "flex", gap: 8, justifyContent: "center", alignItems: "center" }}>
              ...
              <span style={{ fontFamily: "var(--font-mono)", fontSize: 12, color: "var(--text-3)", padding: "0 6px" }}>{page} / {pages}</span>
      ...
      {tab === "scan" && (
        <Card>
          ...
          <p style={{ color: "var(--text-3)", fontSize: 12.5, marginBottom: 18, lineHeight: 1.65 }}>Upload file Excel có nội dung EN / ZH để tự động tìm các cặp thuật ngữ và bổ sung vào từ điển.</p>
          ...
          <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
            ...
            {scanFile && <span style={{ fontFamily: "var(--font-mono)", fontSize: 11, color: "var(--text-2)", background: "rgba(255,255,255,.05)", padding: "5px 10px", borderRadius: 5 }}>{scanFile.name}</span>}
      ...
      {tab === "import" && (
        <Card>
          ...
          <div style={{ display: "flex", gap: 12, marginBottom: 16, flexWrap: "wrap", alignItems: "flex-end" }}>
            {[["Cột nguồn (EN/ZH)", "src"], ["Cột đã dịch (VI)", "tgt"]].map(([lb, key]) => (
              <div key={key}>
                ...
                <TextInput value={impCols[key]} onChange={e => setImpCols(p => ({ ...p, [key]: e.target.value }))} style={{ width: 80 }} />
              </div>
            ))}
          </div>
          ...
          <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
            ...
            {impFile && <span style={{ fontFamily: "var(--font-mono)", fontSize: 11, color: "var(--text-2)", background: "rgba(255,255,255,.05)", padding: "5px 10px", borderRadius: 5 }}>{impFile.name}</span>}
            <Btn variant="solid" color="var(--green)" disabled={!impFile || impBusy} onClick={handleImport}>
              {impBusy ? "⏳ Đang import…" : "📥 Import ngay"}
            </Btn>
          </div>
        </Card>
      )}
...
const HistoryPage = ({ toast }) => {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
      ...
      <Tabs
        tabs={[["all", "Tất cả"], ...Object.entries(ACTION_META).map(([k, v]) => [k, v.label])]}
        ...
        {loading
          ...
          : filtered.length === 0
            ...
            : filtered.map(r => {
              ...
              return (
                <div key={r.id} style={{ background: "var(--bg-2)", border: "1px solid var(--border-1)", borderRadius: 7, padding: "12px 14px", display: "flex", gap: 12, alignItems: "flex-start" }}>
                  <div style={{ width: 7, height: 7, borderRadius: "50%", background: m.color, flexShrink: 0, marginTop: 5 }} />
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: "flex", gap: 7, alignItems: "center", flexWrap: "wrap" }}>
                      <span style={{ fontSize: 10, fontFamily: "var(--font-mono)", background: `${m.color}18`, border: `1px solid ${m.color}30`, color: m.color, borderRadius: 4, padding: "2px 9px" }}>{m.label}</span>
                      <span style={{ color: "var(--text-1)", fontSize: 13, fontWeight: 500, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{r.fileName}</span>
                      {r.targetLang && <span style={{ fontSize: 10, fontFamily: "var(--font-mono)", color: "var(--text-3)", background: "rgba(255,255,255,.05)", padding: "2px 7px", borderRadius: 4 }}>{r.targetLang.toUpperCase()}</span>}
                    </div>
                    {r.details && <div style={{ fontSize: 11, color: "var(--text-3)", fontFamily: "var(--font-mono)", marginTop: 5 }}>{r.details}</div>}
                    {r.outputFile && (
                      <a href={`${API}/translate/download/${r.outputFile}`} download style={{ display: "inline-flex", alignItems: "center", gap: 4, marginTop: 5, fontSize: 11, fontFamily: "var(--font-mono)", color: "var(--accent)", textDecoration: "none" }}>
...
const LogsPage = ({ toast }) => {
  ...
  const loadLogs = useCallback(() => {
    api(`/logs?limit=300${level ? `&level=${level}` : ""}`).then(r => setLogs(r.logs)).catch(() => {});
  }, [level]);
  ...
  const clearLogs = async () => {
    await api("/logs", { method: "DELETE" }); setLogs([]); toast("Đã xóa logs", "success");
  };

  const LC = { error: "#e8394e", warn: "#ffaa30", info: "#00c4e8", debug: "#3a4a5a" };
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14, height: "calc(100vh - 90px)" }}>
      ...
      <div style={{ flex: 1, background: "#04060c", border: "1px solid var(--border-1)", borderRadius: 8, overflowY: "auto", fontFamily: "var(--font-mono)", fontSize: 11.5, padding: "6px 0" }}>
        {logs.length === 0
          ? <div style={{ textAlign: "center", padding: 40, color: "#1e2a38" }}>— no logs —</div>
          : logs.map((log, i) => (
            <div key={i} style={{ display: "flex", gap: 12, padding: "3px 14px", borderBottom: "1px solid rgba(15,20,32,.7)" }}
              onMouseEnter={e => (e.currentTarget.style.background = "rgba(255,255,255,.015)")}
              onMouseLeave={e => (e.currentTarget.style.background = "")}>
              <span style={{ color: "#1e2a38", flexShrink: 0 }}>{new Date(log.timestamp).toLocaleTimeString("vi-VN")}</span>
              <span style={{ color: LC[log.level] || "#3a4a5a", flexShrink: 0, width: 40, fontWeight: 600 }}>{log.level}</span>
              <span style={{ wordBreak: "break-all", lineHeight: 1.5, color: log.level === "error" ? "#f07878" : log.level === "warn" ? "#ffbb60" : "#6a8aaa" }}>{log.message}</span>
            </div>
          ))
        }
      </div>
...
const PAGE_MAP = { translate: TranslatePage, glossary: GlossaryPage, history: HistoryPage, logs: LogsPage };
  ...
  return (
    <>
      ...
      <div style={{ display: "flex", height: "100vh", overflow: "hidden" }}>
        ...
        {/* SIDEBAR */}
        <aside style={{ width: 200, flexShrink: 0, background: "var(--bg-2)", borderRight: "1px solid var(--border-1)", display: "flex", flexDirection: "column" }}>
          <div style={{ padding: "22px 20px 18px", borderBottom: "1px solid var(--border-1)" }}>
            <div style={{ fontFamily: "var(--font-display)", fontSize: 26, lineHeight: 1, color: "var(--text-1)", letterSpacing: 2 }}>
              PFC<span style={{ color: "var(--accent)" }}>HUB</span>
            </div>
            <div style={{ fontSize: 9, fontFamily: "var(--font-mono)", color: "var(--text-3)", marginTop: 3, letterSpacing: 1.5, textTransform: "uppercase" }}>
              Dictionary Builder
            </div>
          </div>
          ...
          <div style={{ padding: "14px 18px", borderTop: "1px solid var(--border-1)" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
              <div style={{ width: 6, height: 6, borderRadius: "50%", flexShrink: 0, background: online === null ? "var(--text-3)" : online ? "var(--green)" : "var(--red)", boxShadow: online === true ? "0 0 6px var(--green)" : undefined }} />
        ...
        <main style={{ flex: 1, overflow: "auto", background: "var(--bg-1)" }}>
          {online === false && (
            <div style={{ background: "rgba(232,57,78,.07)", borderBottom: "1px solid rgba(232,57,78,.2)", padding: "9px 28px", fontSize: 12, fontFamily: "var(--font-mono)", color: "#f07080" }}>
              ⚠ Backend offline — chạy: <code style={{ background: "rgba(255,255,255,.06)", padding: "1px 7px", borderRadius: 4 }}>cd backend &amp;&amp; node --expose-gc server.js</code>
            </div>
          )}
...
body { background: var(--bg-1); color: var(--text-1); font-family: var(--font-ui); font-size: 14px; -webkit-font-smoothing: antialiased; }
...
.page-title { font-family: var(--font-display); font-size: 26px; font-weight: 800; color: var(--text-1); letter-spacing: .5px; line-height: 1; }
.page-desc  { color: var(--text-3); font-size: 13px; margin-top: 5px; }
.field-label { font-size: 10px; font-family: var(--font-mono); color: var(--text-3); text-transform: uppercase; letter-spacing: .8px; margin-bottom: 5px; }

.dropzone { border: 1.5px dashed var(--border-2); border-radius: 10px; padding: 38px 24px; text-align: center; cursor: pointer; background: rgba(255,255,255,.012); transition: border-color .2s, background .2s; }
...
.download-banner { display: flex; align-items: center; gap: 14px; padding: 14px 18px; background: rgba(0,214,138,.07); border: 1px solid rgba(0,214,138,.22); border-radius: 8px; text-decoration: none; color: var(--green); transition: background .2s; }
.download-banner:hover { background: rgba(0,214,138,.12); }

@keyframes toastSlide { from { opacity: 0; transform: translateX(14px); } to { opacity: 1; transform: translateX(0); } }
